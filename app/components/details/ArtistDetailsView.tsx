...
import { Loader2, TrendingUp, Radio, Music, Disc3 } from "lucide-react"
...
export function ArtistDetailsView({ 
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-full p-8">
        ...
        <span className="text-white/80">Caricamento artista...</span>
      </div>
    );
  }
  ...
  if (!artistData) {
    return (
      <div className="text-center p-8">
        <p className="text-white/70">Nessun dato disponibile per questo artista.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-8 pb-32 lg:pb-8">
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 mb-8">
        {/* Immagine artista */}
        <div className="w-48 h-48 lg:w-64 lg:h-64 mx-auto lg:mx-0 rounded-2xl overflow-hidden bg-gradient-to-br from-purple-500 to-pink-500 flex-shrink-0">
          {artist.image ? (
            <img 
              src={artist.image} 
              alt={artist.name}
        ...
        <div className="text-center lg:text-left flex-1">
          <h1 className="text-3xl lg:text-5xl font-bold text-white mb-4">{artist.name}</h1>
      ...
      {topTracks.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xl lg:text-2xl font-semibold text-white mb-4">Brani Popolari</h2>
          <div className="space-y-2">
            {topTracks.map((track, index) => {
              ...
              return (
                ...
                >
                  <span className="text-white/70 font-mono text-sm w-6 text-center">{index + 1}</span>
      ...
      {albums.length > 0 && (
        <div>
          <h2 className="text-xl lg:text-2xl font-semibold text-white mb-4">Album e Singoli</h2>
